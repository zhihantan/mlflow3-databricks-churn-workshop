...
- [ ] **Production-monitoring dashboard (on by default)** — the GenAI experiment's **Overview** charts (Usage / Quality / Tool calls) are populated by **Unity Catalog trace storage**, bound in Module 0 before the first trace. `config/workshop_config.py` auto-resolves a SQL warehouse the runner can use (no pre-configuration), so this is on by default; it requires the workspace's trace-storage preview features. To pin a specific warehouse set the `MONITORING_WAREHOUSE_ID` env var; to turn it off set `WORKSHOP_DISABLE_MONITORING=1`. It also self-disables when no warehouse is usable — the core workshop and the Traces tab work regardless.
...
The Overview dashboards (Usage / Quality / Tool calls) read from **Unity Catalog trace storage**, not the default control-plane store — so the Traces *tab* can be full while the Overview *charts* read 0. This is on by default: Module 0 auto-resolves a SQL warehouse and binds UC trace storage **before the first trace** (a UC trace destination can only bind to a trace-free experiment). If the charts are still empty, check Module 0's output — if it printed "No usable SQL warehouse resolved," the auto-resolver found none this principal can use; pin one with the `MONITORING_WAREHOUSE_ID` env var and re-run from Module 0. If the experiment already has traces from a prior run, delete it and re-run from Module 0. See [Store MLflow traces in Unity Catalog](https://docs.databricks.com/aws/en/mlflow3/genai/tracing/trace-unity-catalog).
